...
<!-- GENERATED by test/corpus/parity-report.ts from plain-parity-sfdp.json — do not edit by hand. -->

# sfdp plain/plain-ext parity dashboard

Differential survey of @knowvah/dot-engine `sfdp` plain/plain-ext output vs the
native `dot -K sfdp` oracle (`test/corpus/plain-walk.ts`), semantic
...
`npx tsx test/corpus/plain-walk.ts sfdp && npx tsx test/corpus/parity-report.ts`.

## Summary

- **Surveyed:** 905
- **pass:** 896 (99.0%) · **diverged (tracked):** 0 · **accepted (documented, won't-fix):** 8
...
_Passing ids (896) are omitted for brevity — the full roster is in
`plain-parity-sfdp.json`._
