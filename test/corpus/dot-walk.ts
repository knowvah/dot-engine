...
//
// `-Tdot` (agwrite) conformance walker.
//
...
// `compareDot` as `{ iterative }` — position-agnostic structural
// comparison for the force-directed engines, exact positional comparison
// (0.01pt tolerance) for the deterministic ones.
//
//   tsx test/corpus/dot-walk.ts <engine> [outJsonl]
//
// Writes dot-parity-<engine>.json (consumed by a later dashboard/report
// task) and appends per-item rows to dot-parity-<engine>.jsonl (or
...
import { compareDot, type DotDiff } from '../golden/compare-dot.js';
...
const RENDER_ONE = join(REPO, 'test/corpus/render-one-dot.ts');
const PARITY = new URL('./parity.json', import.meta.url);
const ACCEPTED = new URL('./accepted-divergences-dot.json', import.meta.url);
 ...
 * coordinate/dimension NUMBERS are skipped by compareDot's `iterative`
...
function oracleSig(engine: EngineName): string {
  ...
  return createHash('sha1')
    .update([DOT_BIN, GVBINDIR, 'dot', String(engine), mt].join('\0'))
    .digest('hex')
    .slice(0, 12);
}

function cacheDir(engine: EngineName): string {
  if (process.env.DOT_ORACLE_CACHE) return process.env.DOT_ORACLE_CACHE;
  return join(tmpdir(), 'dot-corpus-agwrite-oracle', String(engine), oracleSig(engine));
}

const TIMEOUT_MULT = Number(process.env.DOT_TIMEOUT_MULT ?? 3);
const TIMEOUT_FLOOR_MS = Number(process.env.DOT_TIMEOUT_FLOOR_MS ?? 180_000);
const ORACLE_TIMEOUT_MS = Number(process.env.DOT_ORACLE_TIMEOUT_MS ?? 300_000);
const CONCURRENCY = Number(process.env.DOT_CONCURRENCY ?? 8);
const LIMIT = Number(process.env.DOT_LIMIT ?? 0);
...
/** One JSONL row / one entry of dot-parity-<engine>.json's `results`. */
export interface DotWalkResult {
  id: string;
  path: string;
  ...
  verdict: DotVerdict;
  formats: Record<DotFormat, DotFormatResult>;
}

/** dot-parity-<engine>.json shape. */
export interface DotParityReport {
  generatedAt: string;
  ...
  corpusRoot: string;
  total: number;
  counts: Record<DotVerdict, number>;
  results: DotWalkResult[];
}
...
async function oracleFormat(
  ...
  // COMPLETENESS, not exit code: native dot exits nonzero on recoverable
  // warnings (missing image file) while still emitting a complete graph. The
  // plain format ends with a `stop` line; agwrite ends with the closing brace.
  if (r.timedOut || lines[lines.length - 1] !== '}') {
...
function summarize(diffs: DotDiff[]): Pick<DotFormatResult, 'diffCount' | 'firstDiffs'> {
  return {
    diffCount: diffs.length,
    firstDiffs: diffs
      .slice(0, 5)
      .map((d) => `[${d.kind}] ${d.path}: port=${d.port} native=${d.native}`),
  };
}

/** Oracle -> port -> compareDot for one format, folding in acceptance. */
async function resolveFormat(
  ...
  iterative: boolean,
  isAccepted: boolean,
): Promise<{ result: DotFormatResult; diffs: DotDiff[] }> {
  ...
  const { verdict: cmpVerdict, diffs } = compareDot(port.text, oracle.text, { iterative });
  if (cmpVerdict === 'pass') return { result: { verdict: 'pass' }, diffs: [] };
  const verdict: DotVerdict = isAccepted ? 'accepted' : 'diverged';
...
async function walkOne(
  ...
  accepted: Set<string>,
  engine: EngineName,
): Promise<{ result: DotWalkResult; diffs: DotDiff[] }> {
  ...
  // Single format: `-Tdot` has no `-ext` counterpart, so there is no worst-of.
  const r = await resolveFormat(absInput, item.id, 'dot', engine, tsx, iterative, isAccepted);

  return {
    result: { ...meta, verdict: r.result.verdict, formats: { dot: r.result } },
    diffs: r.diffs,
  };
}
...
/** Entry shape (see accepted-divergences-dot.json's `comment`):
...
function defaultJsonlPath(engine: EngineName): string {
  return fileURLToPath(new URL(`./dot-parity-${engine}.jsonl`, import.meta.url));
}

function summaryPath(engine: EngineName): string {
  return fileURLToPath(new URL(`./dot-parity-${engine}.json`, import.meta.url));
}
...
async function runEngineWalk(
  ...
  if (existsSync(outJsonl)) {
    for (const ln of readFileSync(outJsonl, 'utf8').split('\n')) {
      if (!ln) continue;
      try { doneIds.add((JSON.parse(ln) as DotWalkResult).id); } catch { /* partial line */ }
    }
  } else {
  ...
  const results: DotWalkResult[] = [];
  for (const ln of readFileSync(outJsonl, 'utf8').split('\n')) {
    if (!ln) continue;
    try { results.push(JSON.parse(ln) as DotWalkResult); } catch { /* partial line */ }
  }
  const counts: Record<DotVerdict, number> = {
    pass: 0, diverged: 0, accepted: 0, oracleError: 0, portError: 0, timeout: 0,
  };
  for (const r of results) counts[r.verdict]++;
  const report: DotParityReport = {
    generatedAt: new Date().toISOString(),
    generatedWith: 'test/corpus/dot-walk.ts',
  ...
  process.stderr.write(`[${engine}] wrote dot-parity-${engine}.json — ${JSON.stringify(counts)}\n`);
}
...
async function main(): Promise<void> {
  ...
  if (!engineArg) {
    console.error('usage: npx tsx test/corpus/dot-walk.ts <engine> [outJsonlPath]');
    process.exit(2);
    return;
  }
  ...
  process.stderr.write(
    `dot[${engine}] walk: ${items.length} conformant items, size-sorted small→large\n` +
