...
model before the API. dot-engine has three entry points
(`dot-engine`, `@knowvah/dot-engine/api`, `@knowvah/dot-engine/render`, per ADR-2) and a
...
1. **What dot-engine is** (2-3 sentences; DOT in → SVG/geometry out, pure TS,
...
3. **The three entry points — which door?**
   - `dot-engine` (root): quick path — `renderSvg(dot, engine)`, `parse`,
     global config (`setTextMeasurer`, `setImageSizer`, `setImageResolver`).
   - `@knowvah/dot-engine/api`: build a graph in code + read computed geometry
     (`createGraph`, `addEdge`, `getLayout`).
   - `@knowvah/dot-engine/render`: multi-format output + structured draw-ops
     (`render`, `getDrawOps`).
