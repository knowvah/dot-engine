...
|----|-------------|-------|--------|-----------|------|
| T1 | Targeted engine-track refresh + accept closure | (main) | `test/corpus/parity-<engine>.json(l)` ×7, `accepted-divergences-{json,map,plain}.json` (closures only), affected `{json,map}-parity-<engine>.json(l)` | — | [x] |
