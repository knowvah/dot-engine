...
- **Current parity:** `dot` SVG 762/788 conformant (0 unaccepted tracked gaps —
  every remaining non-conformant graph is a documented, accepted divergence);
  `dot` xdot 761/761; `patchwork` xdot 762/762; `osage` xdot 755/762; `circo`
  xdot 752/762; `twopi` xdot 746/762 (all deterministic, ±0.01).
  `neato`/`fdp`/`sfdp` are characterized at ±0.5 rather than gated at the
  deterministic bar, per the tolerance split above. These figures are a
  ...
  [docs-site parity pages](https://knowvah.github.io/dot-engine/engines),
